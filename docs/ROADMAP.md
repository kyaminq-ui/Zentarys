...
| 1.11 | **Arbres et grande végétation** | voie des entités, `docs/systems/02` §5.2 | ✅ | assets, dispersion, montage, et **le tronc écrit dans le terrain** ; la pose des filons, qui appartenait à 2.6, est faite le 2026-09-15 |
...
🔶 **2.6 n'est faite que pour les filons** (2026-09-15, `CWOreScatter`). La
boucle de pose de `WorldInfo_scatterObjectsInArea` est commune aux créatures et
aux plantes (`docs/systems/02` §6), mais rien dans la source ne dit comment
elle choisit entre les deux en un point donné — et l'apparence des créatures
étant hors périmètre du jalon 2, il n'y avait rien à poser de ce côté-là avant
2.1. Ce qui restait déblayé et sans dépendance amont, la pose des neuf filons
(entités 131-139, même espace de codes que la flore), est fait : voir le
journal du 2026-09-15 et `nextsteps.md` §0.
...
🔶 **3.1 a deux tranches : un corps physique, puis un personnage visible en
troisième personne.** La fonction qui ferait autorité pour le mouvement,
`vfunc_10`, est la plus grosse du binaire (77 Ko de pseudo-code) et mélange
indissociablement déplacement, menus, inventaire et artisanat — il n'y a pas de
sous-bloc « physique du joueur » a en extraire. **Décision de ce projet, comme
le réseau de chemins au jalon 1.16** : `CWPlayerController` (`src/demo/
cw_player_controller.gd`) est un `CharacterBody3D` standard Godot — gravité,
marche, saut, collision — dont les constantes de vitesse et de hauteur de saut
n'ont pas de source à citer. Activé par `--joueur`, il **s'ajoute** à la caméra
libre plutôt que de la remplacer : tous les outils de capture (`--ici`,
`--vers`, `--regard`, la téléportation de biome) continuent de la piloter par
défaut.

La seconde tranche lui donne un corps — quatre pièces (`tools/blender/
generer_personnage.py`, `CWPlayerBody`), articulées et animées
proceduralement, sur la plage de palette « créatures » ouverte depuis le
2026-09-05 et peinte ici pour la première fois. La voir suppose la troisième
personne : le joueur passe derrière son personnage plutôt que dans sa tête.
Voir le journal du 2026-09-16 pour les pièges qu'il a fallu fermer avant que ça
tienne debout — un de collision, deux de repère d'axes.
...
| Six biomes à parts égales | ✅ | 2026-09-11 : `tools/biome_balance.gd` résout les seuils par quantiles et le niveau de la mer passe de 0 à −60. Greenlands 41 → 18,5 %, Lava Lands 1,1 → 16,6 %. La règle a changé de **forme** — une seule frontière d'humidité au lieu de deux — parce qu'avec deux le désert était insoluble |
...
| LOD natif (`VoxelLodTerrain`) | ➖ | **2 048 blocs de vue en 16 s**, contre 384 en 23 s à plat (2026-09-13). Fonctionne, mais **pas le défaut** : le rendu à plat est jugé meilleur en jeu. Code conservé, pilotable par `--lod` — voir ci-dessous |
...
**Verdict : le mode plat reste le défaut, par jugement de rendu.**
`TerrainDemo.use_lod` reste à **faux** — non plus faute d'avoir vérifié
l'édition en LOD, mais parce qu'en jeu, manette en main, la scène plate est
jugée plus lisible que la pyramide à six niveaux. Le LOD tient pourtant ses
433 vérifications, et rien de ce qui suit n'a été trouvé en défaut : le rendu,
le chargement, la mémoire, la composition du monde niveau par niveau
(`tests/lod_test.gd`). Ce que ça laisse non vérifié — **l'édition, la
persistance et les collisions en mode LOD** (la couche d'édition est branchée
par `has_method` et non sur la classe, donc elle prend son `VoxelTool` du nœud
sans rien savoir de lui) — cesse d'être un blocage puisque le mode n'est plus
candidat au défaut. Le code n'est pas retiré : il reste pilotable par `--lod`
pour qui voudrait le rejuger un jour.
...
| Date | Fait |
|---|---|
| 2026-09-16 (suite) | **3.1, deuxième tranche : le personnage visible, sur deux images de référence.** Demande en session : un personnage chibi (cheveux blonds hérissés, yeux bleus, tunique sombre, boucle claire), en quatre pièces articulées et animées proceduralement plutôt qu'un modèle figé. Pas de MCP Blender connecté ; la voie prise est celle déjà en place pour la flore, les arbres et les filons — **Python pur**, sans `bpy` (`tools/blender/generer_personnage.py`). **La plage « créatures » (41-95), ouverte le 2026-09-05 et jamais peinte, reçoit son premier contenu réel** (`CWPalette.PLAYER_*`) — l'apparence des créatures étant hors périmètre du jalon 2, rien n'y avait touché jusqu'ici. La tunique n'avait de rampe nulle part : ni les rampes de peau/fourrure/écailles/chitine des créatures, ni les métaux/manches/cuir/gemmes de l'équipement ne sont du tissu porté. **Décision de ce projet, comme `gres`/`cristal_de_glace` pour les filons** : les deux pas les plus sombres de « tissus, bannières » (220-227, plage structures, jalon 4, également vierge) sont repeints en tunique plutôt que d'ouvrir une neuvième rampe. `CWPlayerBody` pose chaque pièce sous un pivot (deux hanches, deux épaules, torse, tête) ; un seul bras et une seule jambe existent sur le disque, l'autre côté est un reflet de nœud. Le joueur passe en **troisième personne** — décision prise en session, faute de modèle de mains pour une vue subjective crédible. **Deux défauts de repère trouvés à la capture, aucun à la lecture du code.** `VoxelVoxLoader` permute les axes en chargeant un `.vox` (`vox(x,y,z) -> godot(y,z,x)`) : écrire largeur et profondeur d'auteur directement dans le fichier les échange a l'arrivée, et le torse (9 de large, 5 de profond) ressortait large de 0,375 bloc vu de face — un vide entre la tête et les jambes qui ressemblait a une pièce manquante. Une fois les axes redressés, le visage regardait **vers l'arrière** : la permutation ne retourne pas le signe de la profondeur, et le personnage avance vers -Z — sans le correctif, marcher en avant l'aurait fait reculer face à la caméra. Les deux sont corrigés au même endroit (`_pose`, dans le générateur), avec la mesure qui les a montrés en commentaire. Un troisième défaut, de collision plutôt que de repère, a précédé les deux autres : le pivot de tangage étant posé à l'intérieur de la capsule du joueur, une première version avec `SpringArm3D` se cognait contre son propre corps et écrasait la caméra à distance quasi nulle — remplacé par un décalage fixe, plus simple, au prix de l'évitement de mur (noté comme défaut connu). Vérifié en jeu sous trois angles : dos cohérent (cheveux, pas d'yeux), face cohérente (yeux bleus, boucle au col). Suite : **449 vérifications, 0 échec**, aucune régression. |
| 2026-09-16 | **3.1, première tranche : un corps physique, invente plutôt que porté.** La fonction qui ferait autorité pour le mouvement du joueur, `GameController::vfunc_10`, est la plus grosse du binaire — 77 Ko de pseudo-code Ghidra, indissociable des menus, de l'inventaire et de l'artisanat. Recherché en amont : cinq fonctions de statistiques de créature (`calc_attack_power`, `calc_stat_variant`, `roll_random_level`...) existent bien pour 2.1, avec une vraie courbe de niveau à rendements décroissants portée verbatim cinq fois — mais aucune table espèce → stats, aucun nom pour les neuf emplacements de stats qu'elles indexent : la session a rebroussé vers 3.1 plutôt que d'inventer une table de stats sans rien à quoi l'accorder. **Décision de ce projet, comme le réseau de chemins au jalon 1.16** : `CWPlayerController` (`src/demo/cw_player_controller.gd`) est un `CharacterBody3D` Godot standard — gravité, marche, saut, capsule de collision à la hauteur de référence du personnage (2,4 blocs, `docs/ASSETS.md`) — activé par `--joueur` et **ajouté** à la caméra libre, qui reste le défaut de tous les outils de capture. Un vrai défaut trouvé à la première capture : le joueur apparaissait en l'air et tombait indéfiniment, jusqu'à `y -1262` pour un sol à `120` — le terrain n'avait pas fini de charger la collision de la colonne de départ, la gravité s'appliquait dans ce vide, et l'observateur de vue suivait la chute au lieu de rester sur place, si bien que le terrain se mettait à charger la colonne *d'arrivée* plutôt que celle de départ. `_floor_within_reach()` sonde toute la hauteur jouable avant d'appliquer la gravité : rien trouvé du tout, ce n'est pas une chute, c'est un chargement, et le corps reste immobile le temps qu'il finisse. Vérifié en jeu (`--joueur --shot 10`) : le joueur atterrit et se stabilise à `y 123` pour un sol à `120`, conforme à la hauteur d'œil de la capsule. `--ici`/`--regard` recoupés en `player_mode` : les fonctions de placement de la démo lisent et écrivent maintenant `camera.global_position`/`global_rotation` plutôt que la position locale, ce qui les rend correctes que la caméra soit un enfant direct de la démo (caméra libre) ou de `CWPlayerController` (joueur). Suite : **449 vérifications, 0 échec**, aucune régression — cette tranche ne touche à rien de la génération. |
| 2026-09-15 | **2.6, l'apparition : les filons affleurent.** La porte du jalon 2 était déblayée depuis le 2026-09-05 — neuf modèles, le tirage de rareté (`CWPalette.roll_ore`), la plage de palette 32-40 — et il ne manquait que « la couche qui les pose », comme `generer_filons.py` le disait en toutes lettres. **Où affleure un filon n'est écrit nulle part dans la source** ; la réponse est venue du modèle lui-même : chaque `.vox` porte sa propre gangue de roche (index 1) autour de sa veine, « pour se fondre dans le terrain au lieu de dessiner un caillou posé sur la paroi ». Un filon n'a donc de sens que là où il y a de la roche à fondre dedans — et la falaise est la seule matière du monde qui en soit (invariant n° 27). `CWOreScatter` reprend l'architecture des arbres (cellule de 64, voisinage 3×3 sans récursion) et porte verbatim les trois constantes extraites de `docs/systems/02` §6 — espacement minimum de 20 blocs, rejet près d'un élément de tuile sauf le donjon, abandon à une chance sur quatre sous 0,2 d'humidité ou de température — mais pas le raster lui-même (pas de 85, décalage +24), qui est la géométrie d'une boucle source et non d'une cellule mise en cache. **Décision de ce projet, notée comme telle** : `gres` et `cristal_de_glace`, absents du tirage de rareté (la source ne dit pas qui les pose), sortent respectivement en Deserts et Snowlands — les deux seuls biomes qui pouvaient leur donner un sens sans inventer un troisième axe. **Un vrai défaut trouvé par la suite existante, pas par la nouvelle** : `tests/lod_test.gd` a viré au rouge sur deux colonnes — un filon affleure jusqu'à quatre blocs au-dessus du sol et ne survit pas à `ORE_MAX_LOD` (zéro, comme un houppier ne survit pas à `TREE_MAX_LOD`), donc sans le couper la suite disait « le sol a disparu » là où seul un affleurement manquait. Corrigé comme les arbres l'avaient été : la couche est coupée dans ce test-là, pas dans le monde. Vérifié en jeu (`tools/find_ore.gd`, graine 2024) : un filon de fer affleure dès le premier flanc rencontré près du point de départ, sandwiché dans la roche de pente exactement comme prévu (`generated_voxel` : roche, filon, filon, roche, air). **Hors périmètre, et ça le reste** : le choix d'une créature contre un filon en un point donné n'est dit nulle part dans la source, et l'apparence des créatures est explicitement hors périmètre du jalon 2 — cette session ne pose que des filons. Suite : **449 vérifications, 0 échec**. |
| 2026-09-14 | **Le monde reste plat par choix, pas par défaut technique.** Le LOD natif tient ses 433 vérifications depuis la veille et rien n'y a été trouvé en défaut — mais vu en jeu, manette en main, la scène plate est jugée plus lisible que la pyramide à six niveaux. `TerrainDemo.use_lod` reste **faux**, et cette fois ce n'est plus faute d'avoir testé l'édition, la persistance et les collisions en LOD (elles restent non vérifiées) : c'est un choix de rendu qui rend cette question sans objet pour l'instant. **Rien n'est retiré** — à la différence de la falaise v1, des ponts, des surplombs et de la plage/du haut-fond/du marais, le LOD n'a échoué à aucune capture, il n'a simplement pas été préféré. `VoxelLodTerrain`, `--lod`, `--lod-vue`, `tests/lod_test.gd` et `CWVoxelGenerator.TREE_MAX_LOD` restent dans le dépôt, pilotables en ligne de commande, pour qui voudrait le rejuger un jour. |
...
| les neuf filons | **matière** (2026-09-15, `CWOreScatter`) | 13 – 32 | fait — posés sur la falaise (invariant n° 27), leur seule matière hors biome |
...
> **Faite pour les filons le 2026-09-15** (`CWOreScatter`, journal du même
> jour) : ce que cette section annonçait comme déblayé — la fonction lue, les
> constantes de pose extraites — l'était bien. Ce qui suit reste vrai pour la
> moitié de 2.6 qui reste à ouvrir : les créatures n'ont ni modèle ni
> comportement avant le jalon 2.1, et rien dans la source ne dit comment cette
> même boucle choisirait entre un filon et une créature en un point donné.
